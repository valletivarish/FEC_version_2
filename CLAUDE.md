...
| 08 retail-footfall-inventory | Adithya Reddy Madireddy | `https://rfi-frontend-964908693441.s3.us-east-1.amazonaws.com/index.html` | `https://gpvjq339tf.execute-api.us-east-1.amazonaws.com/prod` | 200 OK |
...
Live resources in this account (as of 2026-07-19, project 08 only, provisioned via `terraform/`): DynamoDB table `rfi-readings`, SQS queue `rfi-store-agg`, Lambda `rfi-processor` (SQS-triggered ingestion, java17) and Lambda `rfi-dashboard-api` (behind API Gateway REST API `gpvjq339tf`, java17), EC2 instance `i-01d2e59073b93ab86` (tagged `rfi-fog-host`, in `us-east-1a`, runs the fog node + 10 sensor containers, security group `sg-001e820d2dba2a4e1` allows only inbound TCP 8000), Elastic IP `100.55.107.85` (allocation `eipalloc-0db014997dcef595f`, associated with that instance), S3 bucket `rfi-frontend-964908693441` (static dashboard frontend, public read-only, static website hosting enabled) and S3 staging bucket `rfi-deploy-964908693441`. All are prefixed `rfi-`. The dashboard Lambda's `FOG_HEALTH_URL`/`FOG_THRESHOLDS_URL` env vars point at this Elastic IP; if it's ever released and reallocated, they need updating.

Live URLs: dashboard at `https://rfi-frontend-964908693441.s3.us-east-1.amazonaws.com/index.html`, its API at `https://gpvjq339tf.execute-api.us-east-1.amazonaws.com/prod`. The dashboard and its API are fully serverless (S3 + Lambda + API Gateway) and do not depend on the EC2 instance being up; only `/api/health`'s `gateway` field and fresh sensor data depend on fog/sensors running on EC2.
