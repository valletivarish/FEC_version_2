...
Project 15 (data-center-environmental-monitoring) sensor/fog/backend/dashboard implementation and unit tests (114 total) were already complete from an earlier LocalStack-only phase; the real AWS deployment described above is now done and live too, following the same rigor as project 22: two credential-handling bugs proactively found and fixed before deployment (three files silently fell back to hardcoded LocalStack-only credentials, which would have broken authentication in real Lambda/EC2 exactly like project 22's earlier bug — fixed with three genuinely distinct code shapes, not copy-pasted from project 22's fix or from each other), plus the same DynamoDB Scan-pagination undercount bug found and fixed in `pipelineStatus.js`. End-to-end pipeline independently verified live: real sensor data flows through fog, SQS, both Lambdas, DynamoDB, API Gateway, and renders on the S3-hosted dashboard with zero console errors. The technical report for Nithin is not yet started.
