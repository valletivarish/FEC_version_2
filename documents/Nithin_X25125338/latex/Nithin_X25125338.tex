...
Verification of both fixes happened against the live deployment itself, not just in theory: redeploying and then successfully completing a DynamoDB write and SQS receive end to end confirmed the credential fix, while the pagination fix was confirmed by watching the deployed table return a correct item count directly, rather than taking the unit test suite's word for it. Everything described here (source code and fixes alike) sits at \url{https://github.com/nithinveshala/fog-and-edge}, alongside a readme that walks through installation, configuration, and this section's deployment history.
