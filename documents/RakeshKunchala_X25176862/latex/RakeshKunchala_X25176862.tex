...
\caption{Deployed fog-to-cloud topology. A single Amazon EC2 edge host at the plant runs the fog gateway and ten sensor containers, five streams at each of two plants. Solid arrows are the ingest path, from the gateway through Amazon SQS and the processor function into the Amazon DynamoDB table, and the serving path, from the dashboard function's read of the stored windows out through Amazon API Gateway to the browser, with static assets from Amazon S3. The dashed edges are the three further status sources the dashboard polls for its health strip: the gateway's reachability, the queue's depth, and the processor function's deployment state.}
\label{fig:topology}
\end{figure*}
