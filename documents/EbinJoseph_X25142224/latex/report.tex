...
Two simulated slopes, slope-a and slope-b, each carry five sensor types: snowpack depth (snowpack\_depth\_cm), snow temperature (snow\_temp\_c), wind speed (wind\_speed\_kmh), seismic vibration (seismic\_vibration\_mg, a stand-in for the acoustic precursor signal avalanche-monitoring literature associates with slab failure \cite{ref3}), and lift chair count (lift\_chair\_count, an operational rather than a hazard signal). Each sensor process runs two independent, AbortController-coordinated setTimeout loops rather than a single combined timer: one samples a bounded random walk into a local outbox at SAMPLE\_INTERVAL, the other drains that whole outbox in one POST at DISPATCH\_INTERVAL, restoring the batch to the front of the outbox if the request fails so nothing sampled while a dispatch was in flight is lost or reordered.

The fog node buffers every incoming reading in a bare object literal keyed on sensor type and site, not a Map, an array, or a ring buffer: the intake function appends straight into the group matching that key, and on a fixed WINDOW\_SECONDS timer a snapshot-and-clear step walks the object's keys, packages each non-empty key into a group, and resets the buffer to a fresh empty object so readings that land immediately afterward start a new window rather than leaking into the one just closed. Each closed group is reduced to five figures (count, minimum, maximum, average, and the last-arrived reading) and checked against Table~\ref{tab:thresholds}'s four alert rules before it ever leaves the fog node.
...
\hline
seismic\_vibration\_mg & avg $>$ 25 & avalanche\_risk\_detected \\
\hline
wind\_speed\_kmh & avg $>$ 80 & lift\_wind\_halt \\
\hline
snow\_temp\_c & avg $>$ 2 & snowpack\_instability\_risk \\
\hline
snowpack\_depth\_cm & avg $<$ 30 & insufficient\_snow\_coverage \\
\hline
\end{tabular}
\end{table}

Every rule in Table~\ref{tab:thresholds} is a Rule class instance, not a row in a generic tuple array, a case in a switch statement, or a value in a dispatch object: each instance owns one field, one comparison operator, limit, alert key, and sensor type, and carries a check method that tests one summary directly against them. The alert evaluator simply filters the rule list by each rule's own check and maps the survivors to their alert keys: no lookup table sits between a closed window and the rule that judges it. The lift chair count deliberately has no Rule instance at all, so it is reported to the dashboard for operational visibility but can never itself trigger an alert.

The fog gateway's HTTP surface (health, thresholds, and the ingest POST) is dispatched through a switch-on-true statement over a composed method-plus-pathname key string, deliberately avoiding Express, an if/else chain, a tuple table, and a routing trie. Node's event-driven, non-blocking I/O model is what makes a plain HTTP server a reasonable choice here at all: Tilkov and Vinoski describe how Node.js trades a thread-per-connection model for a single event loop plus asynchronous I/O, favouring workloads with many concurrent, short-lived connections over CPU-bound work \cite{ref4}, which matches this gateway's job of accepting frequent small POSTs from ten sensor processes rather than performing heavy computation.
...
The fog gateway's outbound SQS client is an ES6 Proxy wrapping a lazily constructed SQS client: reading any property other than a handful of control methods (configure, useClient, reset, publish, publishBatch) triggers the Proxy's get trap, which builds and caches a real client on first access if nothing has already configured or injected one. Reconfiguring deliberately nulls the cached client back out, so calling configure a second time with a new endpoint rebuilds the client on the next access rather than silently reusing a stale one pointed at the wrong region.

\subsection{Testing Strategy}
Each module carries its own package manifest and Node's built-in test runner, no external test framework anywhere: 13 tests for sensors, 47 for the fog layer, 10 for the backend processor, and 51 for the dashboard. The intake tests cover the fog buffer's group-at-ingest and snapshot-and-reset behaviour directly, and the publisher tests exercise the Proxy's lazy construction and queue-url memoization. The server tests and the Lambda-handler tests each exercise their respective dispatch layers with real request objects rather than unit-testing the underlying route handlers in isolation: the former drive a real Node HTTP server bound to an ephemeral port, while the latter call the exported handler with API-Gateway-shaped event objects and injected fake AWS clients, asserting on the returned status code, headers, and body directly.
