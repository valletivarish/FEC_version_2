...
The fog node is a Python FastAPI service that ingests batches over its /ingest endpoint, buffers them per (sensor type, site) key under an asyncio lock, and flushes every WINDOW\_SECONDS (10s) on a fixed timer independent of buffer size. On each flush, the fog node snapshots and clears the buffer atomically, computes each key's minimum, maximum, average, latest value, and reading count, evaluates the aggregate against the threshold rules in Table~\ref{tab:alerts}, and dispatches every window's aggregates as one batched SendMessageBatch call to Amazon SQS, chunked at the API's ten-entry limit. This keeps the number of outbound network calls bounded regardless of how many (sensor type, site) pairs closed a window at the same moment.
...
\hline
soil\_moisture & avg $<$ 20 & irrigation\_needed \\
\hline
temperature & avg $>$ 35 & heat\_stress \\
\hline
temperature & min $<$ 3 & frost\_risk \\
\hline
humidity & avg $>$ 90 & fungal\_risk \\
\hline
light\_intensity & avg $<$ 1000 & low\_light \\
\hline
rainfall & max $>$ 10 & heavy\_rain \\
\hline
\end{tabular}
\end{table}
...
The window-reduction function in fog/aggregation.py implements the standard windowed minimum, maximum, average, latest, and count aggregation described in Section II. Every other file in this project (the sensor loop, the alert-rule table, the SQS publisher, and the dashboard routes) is an independent implementation written for this domain.
