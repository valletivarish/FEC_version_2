...
Each Maven module carries its own JUnit 5 test suite, broken down by module in Table~\ref{tab:tests}; all 82 currently pass. The gateway and router tests exercise the ingest endpoint and the reflection-driven route dispatch over a real Java HTTP server bound to an ephemeral port, not a unit test of validation logic in isolation. A 16-thread concurrent-ingest test proves the compare-and-swap retry loop never drops a reading under real contention. The thresholds-proxy tests cover both the success path and an unreachable-upstream path for the dashboard's fog-thresholds proxy.

Three tests specifically target the defects described in the next subsection. Two assert that a multi-page DynamoDB scan is counted across all pages rather than only the first, and one asserts that a full flush window is chunked into batched sends rather than issued as individual messages.
...
Nothing in this stack stayed a local simulation by the end: an AWS Academy Learner Lab account in us-east-1 now hosts every piece of it, sensor and fog containers included, alongside the queue, both Lambda functions, DynamoDB, API Gateway, and the S3 frontend, and that shift from emulator to a live account is exactly what turned up two problems Docker Compose and LocalStack alone never would have.
...
Two defects, the DynamoDB pagination undercount and the missing SQS batching, passed the full local test suite and a complete LocalStack-backed integration run undetected. Neither one changes what a single-page scan or a single-message send returns at this project's small local data volume: they only change what gets silently dropped once a real account's table or a flush cycle's group count grows past that boundary, and no local test exercises data at that scale. What actually caught both was pointing the same code at the deployed AWS account and at the rendered dashboard in a real browser, not writing more local tests.

The principal difficulties lay not in the application code but in the AWS Academy Learner Lab account's platform and IAM restrictions, and in verifying the rendered dashboard directly rather than relying on a passing API health check alone. Two extensions would matter most going forward: moving the fog and sensor tier off a continuously running EC2 instance onto a fully event-driven, scheduled-Lambda model, and, were the account's IAM restrictions ever lifted, splitting the shared lab role into two least-privilege roles, one for wcm-processor and one for wcm-dashboard-api.
