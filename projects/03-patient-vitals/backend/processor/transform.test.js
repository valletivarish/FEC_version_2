...
test("toRecord builds a composite sort_key from window_end and site_id", () => {
...
test("toRecord accepts already-parsed objects", () => {
  const record = toRecord({ sensor_type: "spo2", window_end: "e2", site_id: "patient-1" });
  assert.equal(record.sort_key, "e2#patient-1");
  assert.deepEqual(record.alerts, []);
});

test("toRecord defaults site_id when absent", () => {
  const record = toRecord({ sensor_type: "spo2", window_end: "e3" });
