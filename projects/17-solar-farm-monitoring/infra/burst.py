...
# Synthetic type names distinct from the real sensor types so burst traffic never lands in the dashboard's partitions.
...
def main():
    ...
    if wait_for_drain(sqs, queue_url):
        ...
        # A slow drain under LocalStack isn't a broken pipeline; only zero progress is, so assert real throughput happened.
