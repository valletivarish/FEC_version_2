...
def process(message_body):
    ...
    site_id = data.get("site_id", "vessel-a")
    return {
        "sensor_type": data["sensor_type"],
        # window_end alone would collide when vessel-a and vessel-b flush in
        # the same window, since sensor_type is the DynamoDB partition key
        # -- concatenating site_id disambiguates the two vessels.
