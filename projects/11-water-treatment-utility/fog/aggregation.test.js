...
test("condensePlantWindow computes count/min/max/avg/latest over the window", () => {
  const readings = [{ ts: "t0", value: 1.0 }, { ts: "t1", value: 3.0 }, { ts: "t2", value: 2.0 }];
  const summary = condensePlantWindow("turbidity_ntu", "plant-1", "NTU", readings, "start", "end");
...
test("latest is the last-in-order reading, not the maximum value", () => {
  ...
  const summary = condensePlantWindow("chlorine_ppm", "plant-1", "ppm", readings, "start", "end");
...
test("avg is rounded to 3 decimal places", () => {
  ...
  const summary = condensePlantWindow("ph_level", "plant-1", "pH", readings, "start", "end");
  assert.equal(summary.avg, 1.667);
});

test("summary carries sensor_type/site_id/unit/window bounds through unchanged", () => {
  const summary = condensePlantWindow("pressure_bar", "plant-2", "bar", [{ ts: "t0", value: 4 }], "2026-01-01T00:00:00Z", "2026-01-01T00:00:10Z");
...
test("a single reading window has min === max === avg === latest", () => {
  const summary = condensePlantWindow("flow_rate_lps", "plant-1", "L/s", [{ ts: "t0", value: 60 }], "s", "e");
