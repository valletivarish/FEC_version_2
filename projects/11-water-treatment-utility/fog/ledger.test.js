...
test("appendEntry is a plain synchronous array push, no per-key structure", () => {
  const ledger = createLedger();
  appendEntry(ledger, { sensorType: "ph_level", siteId: "plant-1", unit: "pH", ts: "t0", value: 7.1 });
  appendEntry(ledger, { sensorType: "chlorine_ppm", siteId: "plant-2", unit: "ppm", ts: "t0", value: 0.9 });
  assert.equal(ledger.entries.length, 2);
  assert.equal(ledger.entries[0].sensorType, "ph_level");
  assert.equal(ledger.entries[1].sensorType, "chlorine_ppm");
});

test("drainEntries empties the ledger and returns entries in arrival order", () => {
  const ledger = createLedger();
  appendEntry(ledger, { sensorType: "turbidity_ntu", siteId: "plant-1", unit: "NTU", ts: "t0", value: 1 });
  appendEntry(ledger, { sensorType: "turbidity_ntu", siteId: "plant-1", unit: "NTU", ts: "t1", value: 2 });
  const taken = drainEntries(ledger);
...
test("drainEntries returns an empty array when nothing was appended", () => {
  const ledger = createLedger();
  assert.deepEqual(drainEntries(ledger), []);
});

test("groupByKey aggregates entries into per (sensor_type, site_id) groups only at call time", () => {
  const entries = [
...
test("groupByKey keeps two site_ids of the same sensor_type in separate groups", () => {
  const entries = [
...
test("end-to-end: append many raw readings, drain once, then group -- aggregation is deferred entirely to this point", () => {
  ...
  appendEntry(ledger, { sensorType: "ph_level", siteId: "plant-1", unit: "pH", ts: "t1", value: 7.2 });
  appendEntry(ledger, { sensorType: "chlorine_ppm", siteId: "plant-1", unit: "ppm", ts: "t0", value: 0.9 });

  const groups = groupByKey(drainEntries(ledger));
  assert.equal(groups.length, 2);
