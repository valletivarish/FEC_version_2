...
// declarative route table below rather than pulling in a static-file
// middleware package.
...
// Project-specific per-site grouping endpoint: GET /api/apiaries lists both
// apiaries (each with its colony health narrative), GET /api/apiaries/:id
// (path parameter matched by the router's regex fallback) returns just one.
async function handleApiaries(deps, res) {
  sendJson(res, 200, { apiaries: await buildApiarySummaries(deps.doc, deps.tableName) });
}

async function handleApiaryDetail(deps, res, apiaryId) {
  const apiary = await getApiarySummary(deps.doc, deps.tableName, apiaryId);
  if (!apiary) return sendJson(res, 404, { error: `unknown apiary: ${apiaryId}` });
  sendJson(res, 200, apiary);
}
...
function buildRouter(deps) {
  ...
  router.route("GET", "/api/apiaries", async (req, res) => handleApiaries(deps, res));
  router.routeParam("GET", /^\/api\/apiaries\/([a-z0-9-]+)$/, async (req, res, url, match) => handleApiaryDetail(deps, res, match[1]));
