...
QUEUE_NAME="${SQS_QUEUE_NAME:-wrf-fleet-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-wrf-processor}"
TABLE_NAME="${TABLE_NAME:-wrf-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.warehouse.processor.FleetHandler::handleRequest \
