...
QUEUE_NAME="${SQS_QUEUE_NAME:-rfi-store-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-rfi-processor}"
TABLE_NAME="${TABLE_NAME:-rfi-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.retail.processor.StoreHandler::handleRequest \
