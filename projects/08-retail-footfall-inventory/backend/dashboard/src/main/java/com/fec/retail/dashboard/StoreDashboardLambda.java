...
public class StoreDashboardLambda implements RequestHandler<Map<String, Object>, Map<String, Object>> {
    ...
    public StoreDashboardLambda() {
        this(warm());
    }

    // Test seam: drives a caller-supplied app so a stubbed DynamoDbClient can stand in for real AWS.
    StoreDashboardLambda(StoreDashboardApp app) {
        this.routes = Map.of(
            "/api/stores", app.handleStores(),
    ...
    private static synchronized StoreDashboardApp warm() {
        if (warmApp == null) warmApp = new StoreDashboardApp();
        return warmApp;
    }
    ...
    public Map<String, Object> handleRequest(Map<String, Object> event, Context context) {
        ...
        if (handler == null) return json(404, "{\"error\":\"not found\"}", "application/json");
