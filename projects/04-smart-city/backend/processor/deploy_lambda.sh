...
QUEUE_NAME="${SQS_QUEUE_NAME:-fsc-metrics-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-fsc-processor}"
TABLE_NAME="${TABLE_NAME:-fsc-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.smartcity.processor.Handler::handleRequest \
