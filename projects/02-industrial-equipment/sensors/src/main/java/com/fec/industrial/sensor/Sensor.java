...
public class Sensor {
    ...
    record MachineProfile(String unit, double lo, double hi, double start, double step) {}

    static final Map<String, MachineProfile> MACHINE_PROFILES = Map.of(
        "vibration",         new MachineProfile("mm/s", 0.2, 9.0, 2.0, 0.4),
        "motor_temperature", new MachineProfile("C", 30, 110, 65, 3.0),
        "bearing_acoustic",  new MachineProfile("dB", 40, 100, 60, 4.0),
        "rotation_speed",    new MachineProfile("RPM", 800, 3600, 1800, 80),
        "power_draw",        new MachineProfile("kW", 5, 75, 35, 5.0)
    );
    ...
    // Bounded random walk from the previous sample so consecutive readings stay realistically close.
    static double nextSample(double current, MachineProfile profile) {
    ...
    static String encodePayload(String sensorType, String siteId, String unit, List<double[]> readings, List<Instant> timestamps) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        MachineProfile profile = MACHINE_PROFILES.get(sensorType);
        ...
        while (true) {
            value = nextSample(value, profile);
            ...
            if (elapsedSeconds >= dispatchInterval && !buffer.isEmpty()) {
                String body = encodePayload(sensorType, siteId, profile.unit(), buffer, timestamps);
                try {
