...
QUEUE_NAME="${SQS_QUEUE_NAME:-fei-sensor-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-fei-processor}"
TABLE_NAME="${TABLE_NAME:-fei-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.industrial.processor.Handler::handleRequest \
