...
class TestProfileCatalog:
    def test_registers_exactly_the_five_expected_reading_types(self):
        assert set(sensor.REEFER_PROFILES) == EXPECTED_PROFILE_KEYS

    @pytest.mark.parametrize("profile_key", list(sensor.REEFER_PROFILES))
    def test_each_entry_is_a_reading_profile_instance(self, profile_key):
        assert isinstance(sensor.REEFER_PROFILES[profile_key], sensor.ReeferProfile)


class TestSensorDriftBounds:
    @pytest.mark.parametrize("profile_key", list(sensor.REEFER_PROFILES))
    def test_step_stays_within_lo_hi_across_many_iterations(self, profile_key):
        profile = sensor.REEFER_PROFILES[profile_key]
        walk = sensor.SensorDrift(profile)
        assert all(profile.lo <= walk.step() <= profile.hi for _ in range(500))

    def test_initial_value_matches_profile_start(self):
        profile = sensor.REEFER_PROFILES["humidity"]
        assert sensor.SensorDrift(profile).value == profile.start

    def test_single_step_never_exceeds_configured_step_size(self):
        profile = sensor.ReeferProfile(unit="x", lo=0, hi=100, start=50, step=2.0)
        walk = sensor.SensorDrift(profile)
