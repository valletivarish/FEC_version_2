...
/**
 * Parses and validates an /ingest request body, rejecting malformed or
 * missing-field payloads with a descriptive reason instead of throwing an
 * unchecked NPE deep in field access. The caller (TerminalGateway.
 * handleIngest) turns a non-null rejection reason into a 400 response,
 * proven at the real HTTP layer by TerminalGatewayHttpTest.
 */
record IngestValidation(String sensorType, String siteId, String unit, List<Reading> readings) {
    ...
    static IngestValidation parse(JsonNode body) {
        ...
        String siteId = body.has("site_id") && !body.get("site_id").isNull() ? body.get("site_id").asText() : "berth-a";
        ...
        return new IngestValidation(sensorType, siteId, unit, readings);
    }
}
