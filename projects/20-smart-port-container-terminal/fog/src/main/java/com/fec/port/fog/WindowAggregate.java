...
/** Aggregate statistics computed over one flush window's buffered readings for one berth/sensor pair. */
public record WindowAggregate(String sensorType, String siteId, String unit, String windowStart, String windowEnd,
    ...
    public static WindowAggregate of(String sensorType, String siteId, String unit, List<Reading> readings,
        ...
        // Readings are appended to the ledger in arrival order and drained in
        // that same relative order (the ledger is sequence-ordered), so the
        // last list element is the most recently sampled reading -- "latest"
        // is last-in-order, not the max value or the max timestamp.
