...
/** Pure transform from one SQS message body (a BatchPayloadJson aggregate) to a DynamoDB item. */
public class ItemMapper {
    ...
    public static Map<String, AttributeValue> toItem(String messageBody) throws Exception {
        ...
        String siteId = data.has("site_id") ? data.get("site_id").asText() : "berth-a";
        String windowEnd = data.get("window_end").asText();
        // Partition key is sensor_type alone, so without a compound sort key
        // berth-a and berth-b reporting the same sensor_type in the same
