...
QUEUE_NAME="${SQS_QUEUE_NAME:-spc-berth-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-spc-processor}"
TABLE_NAME="${TABLE_NAME:-spc-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.port.processor.TerminalHandler::handleRequest \
