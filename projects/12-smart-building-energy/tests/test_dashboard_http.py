...
def running_server(monkeypatch):
    ...
    fixture = {
        "energy_consumption_kw": [row("energy_consumption_kw", "floor-1", now_iso, 40.0)],
        "co2_ppm": [row("co2_ppm", "floor-1", now_iso, 620.0, unit="ppm")],
    }
...
class TestFloorsEndpoint:
    def test_floors_returns_both_floors_with_readings_and_grade(self, running_server):
        status, body = get(running_server, "/api/floors")
        assert status == 200
        by_site = {f["site_id"]: f for f in body["floors"]}
        assert set(by_site) == {"floor-1", "floor-2"}
        floor1 = by_site["floor-1"]
        assert floor1["readings"]["energy_consumption_kw"]["avg"] == 40.0
        assert floor1["efficiency_grade"] in {"A", "B", "C", "D", "F"}
        assert by_site["floor-2"]["efficiency_grade"] is None


class TestReadingsEndpoint:
    def test_valid_sensor_type_returns_200(self, running_server):
        status, body = get(running_server, "/api/readings?sensor_type=energy_consumption_kw&limit=10")
        assert status == 200
        assert body["sensor_type"] == "energy_consumption_kw"
    ...
    def test_non_integer_limit_returns_400(self, running_server):
        status, _ = get(running_server, "/api/readings?sensor_type=co2_ppm&limit=abc")
        assert status == 400
...
class TestStaticAndMisc:
    def test_index_is_served_at_root(self, running_server):
        ...
        assert b"Smart Building Energy Monitoring" in body
