...
class TestAggregate:
    def test_computes_count_min_max_avg_latest(self):
        ...
        result = aggregation.summarise_window("energy_consumption_kw", "floor-1", "kW", readings, "s", "e")
    ...
    def test_latest_is_last_in_arrival_order_not_the_maximum(self):
        ...
        result = aggregation.summarise_window("co2_ppm", "floor-2", "ppm", readings, "s", "e")
        assert result["latest"] == 5.0
        assert result["max"] == 50.0

    def test_avg_is_rounded_to_three_decimal_places(self):
        readings = make_readings(1.0, 2.0, 2.0)
        result = aggregation.summarise_window("water_usage_lpm", "floor-1", "L/min", readings, "s", "e")
        assert result["avg"] == round(5.0 / 3, 3)

    def test_carries_through_identity_and_window_fields(self):
        readings = make_readings(1.0)
        result = aggregation.summarise_window("hvac_temp_c", "floor-2", "C", readings, "start-iso", "end-iso")
    ...
    def test_single_reading_window(self):
        result = aggregation.summarise_window("occupancy_count", "floor-1", "people", make_readings(42.0), "s", "e")
        assert result == {
