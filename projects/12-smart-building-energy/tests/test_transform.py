...
class TestToItem:
    ...
    def test_sort_key_is_window_end_hash_site_id(self):
        item = transform.to_item(sample_message(window_end="e1", site_id="floor-2"))
        assert item["sort_key"] == "e1#floor-2"

    def test_two_floors_in_the_same_flush_get_distinct_sort_keys(self):
        item_a = transform.to_item(sample_message(window_end="e1", site_id="floor-1"))
        item_b = transform.to_item(sample_message(window_end="e1", site_id="floor-2"))
    ...
    def test_carries_alerts_through(self):
        item = transform.to_item(sample_message(alerts=["peak_load_warning"]))
