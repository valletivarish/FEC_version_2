...
# Synthetic type names distinct from the 5 real sensor types, so burst traffic never lands in the partitions the live dashboard reads.
...
def main():
    ...
    if wait_for_drain(sqs, queue_url):
        ...
        # A slow drain under LocalStack isn't a broken pipeline; making zero progress is, so assert real throughput happened even when the drain isn't complete.
