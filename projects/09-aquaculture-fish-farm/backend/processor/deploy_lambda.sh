...
QUEUE_NAME="${SQS_QUEUE_NAME:-aff-pond-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-aff-processor}"
TABLE_NAME="${TABLE_NAME:-aff-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.aquaculture.processor.PondHandler::handleRequest \
