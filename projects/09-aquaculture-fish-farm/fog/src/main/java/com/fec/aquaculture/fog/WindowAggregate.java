...
/** Aggregate statistics computed over one flush window's buffered readings for one pond/sensor pair. */
public record WindowAggregate(String sensorType, String siteId, String unit, String windowStart, String windowEnd,
    ...
    public static WindowAggregate of(String sensorType, String siteId, String unit, List<Double> values,
        ...
        // Values are appended to the accumulator in arrival order and never
        // reordered, so the last element is the most recently sampled
        // reading -- reported separately from the window's min/max/avg as
        // "latest" (last-in-order, not max-timestamp).
