...
QUEUE_NAME="${SQS_QUEUE_NAME:-ptf-depot-agg}"
FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-ptf-processor}"
TABLE_NAME="${TABLE_NAME:-ptf-readings}"
...
if aws --endpoint-url "$ENDPOINT" lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
  ...
  aws --endpoint-url "$ENDPOINT" lambda create-function \
    ...
    --handler com.fec.transit.processor.TransitHandler::handleRequest \
