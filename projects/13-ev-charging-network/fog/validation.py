"""Input validation for POST /ingest, kept separate from app.py so it is
directly unit-testable without booting a real server. Deliberately
permissive on sensor_type -- it does not check against a fixed set, so a
new charging-bay sensor type can be added without a matching edit here."""


def validate_batch(payload):
    """Return an error message string if payload is not a well-formed
    ingest batch, or None if it is fine to accept. Never raises -- the
    caller turns a non-None return into a 400 response."""
    ...
    site_id = payload.get("site_id", "hub-1")
